...
# Parse command line file paths store in variables, output filename for coge-formatted is optional

parser = ArgumentParser(description='CoGe-ifier for Bismark converts context-specific methylation call files '
                                    'generated by Bismark ito .csv format for compatibility with CoGe\'s '
                                    'LoadExperiment Functionality')
parser.add_argument('-i', dest='input_file', required=True, help='Input Bismark context-specific methylation calls',
                    metavar='File')
methyl_path = parser.parse_args().input_file
file_validity(parser, methyl_path)  # Check validity of input filename

parser.add_argument('-o', dest='output_file', required=False, default='%s.coge.csv' % methyl_path,
                    help='Output .csv file for CoGe-mport', metavar='File')
coge_path = parser.parse_args().output_file
file_validity(parser, coge_path)  # Check validity of output filename
